/**
 * Einzelner Eigentümer-Eintrag eines Flurstücks — ändern und entfernen.
 *
...
async function gehoertZumMandanten(
  ...
  const eintrag = await prisma.plotOwner.findFirst({
...
export async function PATCH(
  ...
  try {
    ...
    await prisma.plotOwner.update({
      where: { id: eintragId },
      data: {
        ...(daten.sharePercent !== undefined && {
    ...
    logger.error({ err: error }, "Error updating plot owner");
...
export async function DELETE(
  ...
  try {
    ...
    await prisma.plotOwner.delete({ where: { id: eintragId } });

    return NextResponse.json({ data: { id: eintragId } });
  } catch (error) {
    logger.error({ err: error }, "Error deleting plot owner");
