...
// GET /api/energy/productions/sample-turbine-csv
// Generates a sample CSV for turbine production data (different format from Netzbetreiber)
  ...
  try {
    ...
    if (turbines.length === 0) {
      return NextResponse.json(
        {
          error:
            "Keine Anlagen gefunden. Bitte legen Sie zuerst Anlagen an.",
        },
        { status: 404 }
      );
    }

    // Monthly production pattern (relative factors: winter=high, summer=low)
    const monthlyFactors = [
      0.85, 0.92, 0.88, 0.65, 0.55, 0.48, 0.42, 0.52, 0.68, 0.82, 0.9, 0.95,
    ];
    const baseProduction = 250000; // kWh base
    ...
    return new NextResponse(csvContent, {
      ...
      headers: {
        ...
        "Content-Disposition":
          "attachment; filename=turbinendaten_beispiel.csv",
      },
    });
  } catch (error) {
    logger.error({ err: error }, "Error generating sample turbine CSV");
    return NextResponse.json(
